...
# file: !/home/nedc/Documents/Temple_Analytics/PAB_test05.py
#
# revision history: 
#  20151019 (WO): initial version 
#
...
# attracted the most ticket orders as well as the performance dates to find more detailed info.
# These outputs are then written to .csv files to be input into Piktochart to be made into 
...
def main(argv):
    ...
    try:
        ...
        # Hand off performance description histogram to separate function
        # so that a .csv file can be generated to be input into Piktochart 
        #
        generate_csv(perf_desc_hist, 'perf_desc_hist')

        # Hand off performance date histogram to separate function so that
        # a .csv file can be generated to be input into Piktochart 
        #
        generate_csv(perf_dt_hist, 'perf_dt_hist')
...
# This function writes its argument dictionary to a .csv file
#
def generate_csv(dict_arg, dict_name):

    # Establish writer object pointing to a .csv file named after 
    # the argument dictionary
    #
    writer = csv.writer( open( '%s.csv' % dict_name, 'wb' ) )

    # For each key and value within the argument dictionary 
    #
    for key,value in dict_arg.items():
        
        # Write each row with the key and value as comma separated variables
        #
